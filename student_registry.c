...
int readInt(const char *prompt, int min, int allowMin, int max, int allowMax) {
    ...
    for (;;) {
        ...
        if (result == EOF) { printf("\nInput stream closed unexpectedly. Exiting.\n"); exit(1); }
        if (result != 1) { printf("Invalid input. Please enter a whole number.\n"); clearInputBuffer(); continue; }
        clearInputBuffer();
        if (allowMin && value < min) { printf("Value must be at least %d.\n", min); continue; }
        if (allowMax && value > max) { printf("Value must be at most %d.\n", max); continue; }
        return value;
    }
}
...
float readFloat(const char *prompt, float min, float max) {
    ...
    for (;;) {
        ...
        clearInputBuffer();
        if (value < min || value > max) { printf("Value must be between %.2f and %.2f.\n", min, max); continue; }
        return value;
    }
}
...
void readLine(const char *prompt, char *buffer, int len) {
    for (;;) {
        printf("%s", prompt);
        if (!fgets(buffer, len, stdin)) { buffer[0] = '\0'; return; }
        buffer[strcspn(buffer, "\n")] = '\0';
        if (buffer[0] == '\0') { printf("This field cannot be empty. Please try again.\n"); continue; }
        return;
    }
}
...
void addStudent(Registry *reg) {
    ...
    Student s;
    printf("\n-- Add New Student --\n");
    s.id = readInt("Enter Student ID: ", 1, 1, 0, 0);
    for (int i = 0; i < reg->count; i++) {
        if (reg->students[i].id == s.id) {
            printf("A student with ID %d already exists. Student not added.\n", s.id);
            return;
        }
    }
    readLine("Enter Full Name: ", s.name, NAME_LEN);
    readLine("Enter Major: ", s.major, MAJOR_LEN);
    s.gpa = readFloat("Enter GPA (0.0 - 4.0): ", MIN_GPA, MAX_GPA);
    s.credits = readInt("Enter Credits Taken: ", 0, 1, 0, 0);
    reg->students[reg->count++] = s;
    printf("Student '%s' added successfully! (%d/%d students in registry)\n", s.name, reg->count, MAX_STUDENTS);
}
...
void displayAllStudents(const Registry *reg) {
    ...
    if (reg->count == 0) { printf("No students in the registry yet.\n"); return; }
    printStudentHeader();
    for (int i = 0; i < reg->count; i++) printStudentRow(reg->students[i]);
}

void searchById(const Registry *reg) {
    if (reg->count == 0) { printf("No students in the registry yet.\n"); return; }
    int id = readInt("Enter Student ID to search: ", 1, 1, 0, 0);
    for (int i = 0; i < reg->count; i++) {
        if (reg->students[i].id == id) {
            const Student *s = &reg->students[i];
            printf("\n-- Student Found --\n");
            printf("ID:      %d\nName:    %s\nMajor:   %s\nGPA:     %.2f\nCredits: %d\n",
                   s->id, s->name, s->major, s->gpa, s->credits);
            return;
...
void searchByGpaThreshold(const Registry *reg) {
    ...
    printStudentHeader();
    for (int i = 0; i < reg->count; i++) {
        if (reg->students[i].gpa >= threshold) { printStudentRow(reg->students[i]); matches++; }
    }
    if (matches == 0) printf("(No students met this GPA threshold.)\n");
...
void searchByMajor(const Registry *reg) {
    if (reg->count == 0) { printf("No students in the registry yet.\n"); return; }
    char major[MAJOR_LEN];
    readLine("Enter major to search for: ", major, MAJOR_LEN);
    int matches = 0;
    printf("\n-- Students majoring in '%s' --\n", major);
    printStudentHeader();
    for (int i = 0; i < reg->count; i++) {
        if (strcaseequal(reg->students[i].major, major)) { printStudentRow(reg->students[i]); matches++; }
    }
    if (matches == 0) printf("(No students found with this major.)\n");
    else { printf("--------------------------------------------------------------------\n"); printf("%d student(s) found.\n", matches); }
}
...
void showStatistics(const Registry *reg) {
    ...
    float sumGpa = 0.0f;
    int totalCredits = 0;
    const Student *hi = &reg->students[0], *lo = &reg->students[0];
    for (int i = 0; i < reg->count; i++) {
        sumGpa += reg->students[i].gpa;
        totalCredits += reg->students[i].credits;
        if (reg->students[i].gpa > hi->gpa) hi = &reg->students[i];
        if (reg->students[i].gpa < lo->gpa) lo = &reg->students[i];
    }
    printf("\n-- Class Statistics --\n");
